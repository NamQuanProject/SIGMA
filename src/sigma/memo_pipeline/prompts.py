"""MEMO's own reflection-synthesis prompts, ported near-verbatim from
``MeMo/data_synthesis_pipeline/general_prompt_utils.py`` (the official MEMO paper code
release). Wording/structure is preserved as closely as possible -- these are the actual
...
``memo_pipeline/pipeline.py`` and ``memo_pipeline/llm.py`` for that.
"""
...
def extract_doc_metadata(text: str) -> str:
    """Pull a ``date: ...`` line out of the document text if one is present (MEMO's raw
    corpora sometimes prepend a metadata header); falls back to a placeholder the prompts
    already know how to handle otherwise. Ported as-is from MEMO's ``extract_doc_metadata``.
    """

    pattern = r"(\w+):\s*(.+)"
    matches = re.findall(pattern, text)
    metadata = {key: value.strip() for key, value in matches}
    return metadata.get("date", "No date provided in document")


def prepare_prompt_for_direct_fact_extraction_v3(chunk_content: str, doc_extracted_date: str) -> str:
    return f"""
...
def prepare_prompt_for_indirect_fact_extraction(chunk_content: str, doc_extracted_date: str) -> str:
    return f"""
...
def prepare_prompt_for_consolidation(qa_pairs: str, doc_extracted_date: str) -> str:
    return f"""
...
def prepare_prompt_for_entity_surfacing(qa_pairs_str: str, doc_extracted_date: str) -> str:
    """Given ALL self-contained QA pairs for a document, produce entity-surfacing QA
    pairs where the question describes an entity through its facts without naming it,
    and the answer reveals the entity's full name.
    """
...
def _format_anchor_for_prompt(anchor_qa: dict, anchor_doc_id: str) -> str:
    return f"[Doc {anchor_doc_id}]\nQ: {anchor_qa.get('question', '')}\nA: {anchor_qa.get('answer', '')}"


def _format_other_qa_batch_for_prompt(batch: list[tuple[str, dict]]) -> str:
    """Format a batch of (doc_id, qa_pair) tuples into a labeled block, grouping
    consecutive entries by doc_id for readability.
    """
